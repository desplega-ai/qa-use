...
- **`qa-use tunnel start` (without `--hold`) hangs for the full registry grace window.** The command calls `registry.release()` and the grace timer is `.unref()`'d (`lib/tunnel/registry.ts:446-448`), but the underlying `@desplega.ai/localtunnel` TCP connection keeps the event loop alive until the grace timer fires (default 30s) and triggers teardown. Manually reproduced with `time bun run cli tunnel start http://localhost:3000`: exits in ~0.8s when `QA_USE_TUNNEL_GRACE_MS=100`, ~30s otherwise. Plan intent was "acquires and immediately releases, prints URL and returns." Fix: in `src/cli/commands/tunnel/start.ts:49`, when we own the handle and refcount drops to 0 after release, call `forceClose` to tear down immediately. File a follow-up ticket; low priority (workaround is `--hold`).
