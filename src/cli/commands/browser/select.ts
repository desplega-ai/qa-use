/**
 * qa-use browser select - Select dropdown option
 */
...
export const selectCommand = new Command('select')
  ...
  .argument('<ref>', 'Element ref (e.g., "e5" or "@e5")')
  .argument('<value>', 'Option value to select')
  .option('-s, --session-id <id>', 'Session ID (auto-resolved if only one session)')
  .action(async (ref: string, value: string, options: SelectOptions) => {
    try {
      // Load configuration
      ...
      if (result.success) {
        ...
        console.log(success(`Selected "${displayValue}" in ${normalizedRef}`));
