...
- [x] **Issue #1 — stale annotation path is effectively dead code (severity: minor).** RESOLVED in commit `88692ed` — `shouldSweep()` now skips `browser status` invocations, so users see the current on-disk state (including the `(stale — run qa-use doctor)` annotation) when they ask for it. Verified: seeded stale entry now renders with annotation; no sweep notice; file persists after command.

- [x] **Issue #2 — doctor silently "reaps" stale session when filename ≠ internal id (severity: minor).** RESOLVED in commit `961c736` — `removeSessionRecord(id)` now falls back to a dir scan when the id-derived path hits ENOENT, matching on internal `id` field. Fast path (filename == id) unchanged. Verified: seeded drifted file `qa-stale-DRIFT.json` (internal id `qa-stale-1`) reaped cleanly by `doctor`; sessions dir empty after run.

- [x] **Issue #3 — `tunnel close` leaves `tunnel start --hold` parent process running (severity: minor).** RESOLVED in commit `a9e4aa2` — after `forceClose`, if `record.pid` is still alive and no session was reaped, `tunnel close` now prints a stderr hint with the lingering PID and `kill <pid>` recipe (or includes `holderPid` in the JSON output). Plan's design choice (don't auto-SIGTERM non-session holders) preserved. Verified live: after closing a `tunnel start --hold` tunnel, stderr shows `ℹ Note: holder process PID 15176 still running — send SIGTERM (kill 15176) to terminate.` — PID matches the registry holder.

All three have been fixed + verified. Full suite green at 508/508.
